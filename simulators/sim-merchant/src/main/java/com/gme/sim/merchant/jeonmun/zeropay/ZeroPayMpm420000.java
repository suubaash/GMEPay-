...
/**
 * Builds and parses the 변동형 MPM 결제 전문 (거래구분코드 420000) — the message a 결제사업자
 * (here GMEPay+) sends to the KFTC ZeroPay relay after scanning a merchant dynamic QR.
 *
 * <p>In the merchant terminal this is rendered as the "wire view": the exact 1,000-byte
 * frame that <em>this charge</em> produces on the ZeroPay network, field-by-field.
 */
public final class ZeroPayMpm420000 {
    ...
    /** Builds the field-value map for a 0200 payment request. */
    public static Map<Integer, String> requestValues(PaymentRequest r) {
    ...
    public static byte[] encodePayment(PaymentRequest r) {
        return JeonmunCodec.encode(MPM_DYNAMIC_420000, requestValues(r), CHARSET, ONLINE_MESSAGE_LENGTH);
    }

    /** Annotated, field-by-field view of a 0200 request (for the terminal wire panel). */
    public static List<JeonmunCodec.AnnotatedField> annotatePayment(PaymentRequest r) {
        return JeonmunCodec.annotate(MPM_DYNAMIC_420000, requestValues(r), CHARSET);
    }

    /**
     * Decodes a 0210 response. {@code approved} is a heuristic (blank or "000" response
     * code): the authoritative success/decline codes live in the KFTC 응답코드 table.
     */
