...
public class RestCommittedFxTransactionPort implements CommittedFxTransactionPort {
    ...
    public RestCommittedFxTransactionPort(
        ...
        // T3-11 defect 1: this was `RestClient.builder()` — the STATIC factory, which returns a fresh
        // builder that no RestClientCustomizer has ever touched, so the fleet's connect/read floor in
        // HttpClientTimeoutAutoConfiguration never reached it and this client had NO read timeout at
        // all. `builder` is the injected BEAN and does carry it.
        //
        // Not the money path, but a scheduler path: this read feeds the BOK report generated by a
        // @Scheduled job, and an unbounded read there parks a scheduler thread — which is the same
        // pool the SchedulerLagProbe heartbeat rides on, so the one metric that would report the stall
        // is what the stall hides. The 10s fleet floor is deliberately the loosest value in the
        // platform because a regulatory day-range query over transaction-mgmt is legitimately slow;
        // its job is to convert unbounded into bounded, not to be tight.
