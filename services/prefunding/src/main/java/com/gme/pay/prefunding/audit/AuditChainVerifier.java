...
/**
 * Reads prefunding's {@code audit_log} back and re-derives the hash chain, so the tamper-evidence is
 ...
 * recomputes the digests. Without a read path, "prefunding's audit trail is hash-chained" is an
 * unfalsifiable claim about columns nobody reads, and a DBA who rewrote the actor on a USD 2m credit
 ...
 *       and {@code actor_ip}. This table is created empty by V010 so the count should be 0; it is
 ...
 *   <li><b>Deletion of an entire aggregate.</b> Remove every row for one partner and the key vanishes
