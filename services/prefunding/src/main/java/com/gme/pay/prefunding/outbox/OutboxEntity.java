 ...
 * Same transactional-Outbox pattern as revenue-ledger / transaction-mgmt: rows are written
 * in the SAME transaction as the business write that produced the event (here: a
 * {@code balance_alert} insert) and drained asynchronously by {@link OutboxPublisher}.
 *
 * <p>Mapping mirrors {@code V004__create_outbox.sql}. {@code publishedAt} is {@code null}
 * until the publisher has handed the event to {@link com.gme.pay.events.EventPublisher}
 * successfully; failures leave it null so the next poll tick retries.
 */
@Entity
@Table(name = "outbox")
