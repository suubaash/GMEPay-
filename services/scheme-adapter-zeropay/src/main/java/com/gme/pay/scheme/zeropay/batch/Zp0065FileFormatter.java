package com.gme.pay.scheme.zeropay.batch;

import java.math.BigDecimal;
import java.nio.charset.StandardCharsets;
import java.time.LocalDate;
import java.util.List;

import static com.gme.pay.scheme.zeropay.batch.ZpFileFormatUtils.DATE_FMT;
import static com.gme.pay.scheme.zeropay.batch.ZpFileFormatUtils.TIME_FMT;
import static com.gme.pay.scheme.zeropay.batch.ZpFileFormatUtils.padLeftZero;
import static com.gme.pay.scheme.zeropay.batch.ZpFileFormatUtils.padRight;

/**
 * Formats a ZP0065 payment-detail file as a fixed-width text file (SCH-06 §8.1, ~22:00 KST).
 *
 * <p>Header fixed-width layout (45 chars, same as ZP0011):
 * <pre>
 *  Offset  Width  Field
 *  0       6      "ZP0065"
 *  6       8      businessDate YYYYMMDD
 *  14      10     gmeInstitutionCode CHAR(10)
 *  24      6      totalRecordCount NUM(6)
 *  30      15     totalPayoutAmountKrw NUM(15)
 * </pre>
 * </p>
 *
 * <p>Trailer: same 22-char layout as ZP0011 (T + NUM(6) count + NUM(15) sum).</p>
 *
 * <!-- TODO: confirm header/trailer widths from BS-07 §8.1 once formal spec received -->
 */
public final class Zp0065FileFormatter {

    static final String FILE_TYPE_VALUE          = "ZP0065";
    static final int    FILE_TYPE_LEN            = 6;
    static final int    BUSINESS_DATE_LEN        = 8;
    static final int    INSTITUTION_CODE_LEN     = 10;
    static final int    HEADER_RECORD_COUNT_LEN  = 6;
    static final int    HEADER_TOTAL_AMOUNT_LEN  = 15;
    static final int    HEADER_RECORD_LENGTH     = 45;

    static final int    TRAILER_RECORD_COUNT_LEN = 6;
    static final int    TRAILER_CONTROL_SUM_LEN  = 15;
    static final int    TRAILER_RECORD_LENGTH    = 22;

    private final String gmeInstitutionCode;

    public Zp0065FileFormatter(String gmeInstitutionCode) {
        this.gmeInstitutionCode = gmeInstitutionCode;
    }

    /**
     * Formats a complete ZP0065 file from payment detail records.
     *
     * @param businessDate KST business date for the file
     * @param records      payment detail records; may be empty
     * @return UTF-8 bytes of the complete fixed-width file
     */
    public byte[] format(LocalDate businessDate, List<Zp0065Record> records) {
        BigDecimal totalPayout = records.stream()
                .map(Zp0065Record::payoutAmountKrw)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

        StringBuilder sb = new StringBuilder();
        sb.append(formatHeader(businessDate, records.size(), totalPayout));
        sb.append('\n');

        for (Zp0065Record r : records) {
            sb.append(formatDetail(r));
            sb.append('\n');
        }

        sb.append(formatTrailer(records.size(), totalPayout));
        return sb.toString().getBytes(StandardCharsets.UTF_8);
    }

    // -----------------------------------------------------------------------
    // Package-private for testing
    // -----------------------------------------------------------------------

    String formatHeader(LocalDate businessDate, int recordCount, BigDecimal totalPayout) {
        StringBuilder h = new StringBuilder(HEADER_RECORD_LENGTH);
        h.append(padRight(FILE_TYPE_VALUE,           FILE_TYPE_LEN));
        h.append(businessDate.format(DATE_FMT));
        h.append(padRight(gmeInstitutionCode,        INSTITUTION_CODE_LEN));
        h.append(padLeftZero(String.valueOf(recordCount), HEADER_RECORD_COUNT_LEN));
        h.append(padLeftZero(totalPayout.toBigIntegerExact().toString(), HEADER_TOTAL_AMOUNT_LEN));
        return h.toString();
    }

    String formatDetail(Zp0065Record r) {
        StringBuilder d = new StringBuilder(Zp0065Record.RECORD_LENGTH);
        d.append('D');
        d.append(padRight(r.gmeTxnId(),           Zp0065Record.GME_TXN_ID_LEN));
        d.append(padRight(r.zeroPayTxnRef(),       Zp0065Record.ZP_TXN_REF_LEN));
        d.append(padRight(r.merchantId(),          Zp0065Record.MERCHANT_ID_LEN));
        d.append(padRight(r.qrCodeId(),            Zp0065Record.QR_CODE_ID_LEN));
        d.append(r.txnDate().format(DATE_FMT));
        d.append(r.txnTime().format(TIME_FMT));
        d.append(padLeftZero(r.payoutAmountKrw().toBigIntegerExact().toString(),
                Zp0065Record.PAYOUT_AMT_LEN));
        d.append(padLeftZero(r.merchantFeeAmt().toBigIntegerExact().toString(),
                Zp0065Record.MERCHANT_FEE_LEN));
        d.append(padLeftZero(r.vanFeeAmt().toBigIntegerExact().toString(),
                Zp0065Record.VAN_FEE_LEN));
        d.append(r.partnerType());
        d.append(padRight(r.approvalCode(),        Zp0065Record.APPROVAL_CODE_LEN));
        d.append(r.statusCode());
        d.append(r.settlementDate().format(DATE_FMT));
        d.append(padRight("", Zp0065Record.RESERVED_LEN));
        return d.toString();
    }

    String formatTrailer(int recordCount, BigDecimal controlSum) {
        StringBuilder t = new StringBuilder(TRAILER_RECORD_LENGTH);
        t.append('T');
        t.append(padLeftZero(String.valueOf(recordCount), TRAILER_RECORD_COUNT_LEN));
        t.append(padLeftZero(controlSum.toBigIntegerExact().toString(), TRAILER_CONTROL_SUM_LEN));
        return t.toString();
    }
}
