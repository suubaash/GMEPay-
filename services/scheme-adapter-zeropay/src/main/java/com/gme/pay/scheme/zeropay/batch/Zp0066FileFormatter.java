...
/**
 * Formats a ZP0066 refund-detail file as a fixed-width text file (SCH-06 §8.1, ~22:00 KST).
 *
 * <p>Header and trailer layouts are identical to ZP0065 except the 6-char type code
 * {@code "ZP0066"}.</p>
 *
 * <!-- TODO: confirm from BS-07 §8.1 once received -->
 */
public final class Zp0066FileFormatter {

    static final String FILE_TYPE_VALUE          = "ZP0066";
    ...
    public Zp0066FileFormatter(String gmeInstitutionCode) {
        this.gmeInstitutionCode = gmeInstitutionCode;
    }

    /**
     * Formats a complete ZP0066 file from refund detail records.
     *
     * @param businessDate KST business date for the file
     * @param records      refund detail records; may be empty
     * @return UTF-8 bytes of the complete fixed-width file
     */
    public byte[] format(LocalDate businessDate, List<Zp0066Record> records) {
    ...
    String formatDetail(Zp0066Record r) {
        StringBuilder d = new StringBuilder(Zp0066Record.RECORD_LENGTH);
        d.append('D');
        d.append(padRight(r.gmeTxnId(),             Zp0066Record.GME_TXN_ID_LEN));
        d.append(padRight(r.zeroPayTxnRef(),         Zp0066Record.ZP_TXN_REF_LEN));
        d.append(padRight(r.merchantId(),            Zp0066Record.MERCHANT_ID_LEN));
        d.append(padRight(r.qrCodeId(),              Zp0066Record.QR_CODE_ID_LEN));
