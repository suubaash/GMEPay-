-- V023 (H2 VENDOR VARIANT): partner_corridor child table — Slice 7
-- "Schemes & Corridors" (docs/PARTNER_SETUP_PLAN.md §Slice 7).
--
-- ENGINE SPLIT (same mechanism as V004)
-- -------------------------------------
-- This file is the H2 twin of db/vendor/postgresql/V023__partner_corridor.sql.
...
-- keyword (H2 computed columns are always stored) — there is no single
...
-- application.properties. Keep the two files in lock-step when either changes.
--
-- WHY
-- ---
...
CREATE TABLE partner_corridor (
    ...
    -- UPDATE vacates the index slot automatically. H2 form: NO `STORED`
    -- keyword (H2 rejects it; its computed columns are always stored).
    is_current     BOOLEAN GENERATED ALWAYS AS
                       (CASE WHEN superseded_at IS NULL THEN TRUE END),
