-- V004 (H2 VENDOR VARIANT): bitemporal effective dating on the partners
-- aggregate (ADR-010).
--
-- ENGINE SPLIT (Slice 6 fix)
-- --------------------------
-- This file is the H2 twin of db/vendor/postgresql/V004__partner_bitemporal.sql.
-- The two differ in EXACTLY ONE token: the `STORED` keyword on the generated
-- column in step (7). PostgreSQL 12+ REQUIRES `GENERATED ALWAYS AS (...)
-- STORED`; H2 (2.2 and 2.3 alike, in every compatibility mode) REJECTS the
-- STORED keyword with a JdbcSQLSyntaxErrorException — there is no single
-- spelling both engines parse. Flyway resolves the right variant through the
-- vendor-substituted location `classpath:db/vendor/{vendor}` configured in
-- application.properties. The PostgreSQL variant is byte-identical to the
-- file every live PG database already applied, so its recorded checksum in
-- flyway_schema_history stays valid; H2 databases are always fresh in-memory
-- instances, so this variant carries no checksum-compatibility burden.
--
-- Everything below this header block mirrors the PostgreSQL variant — keep
-- the two in lock-step when either changes.
--
-- WHY
-- ---
...
-- H2 form: NO `STORED` keyword — H2 rejects it in every mode (verified against
-- 2.2.224 and 2.3.232); H2 computed columns are always stored. The PostgreSQL
-- vendor variant of this file carries the `STORED` keyword PG requires.
