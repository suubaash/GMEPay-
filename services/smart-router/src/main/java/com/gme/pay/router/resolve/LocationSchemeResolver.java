 ...
 * <ol>
 *   <li>{@link ResolutionError#VALIDATION_ERROR} — null/blank country, missing
 *       mode, or a direction outside {@link Direction}.</li>
 *   <li>{@link ResolutionError#NO_SCHEME_FOR_LOCATION} — no enabled row exists
 *       for the country at all.</li>
 *   <li>{@link ResolutionError#DIRECTION_NOT_ENABLED} — rows exist for the
 *       country, but none participates in the requested direction.</li>
 *   <li>{@link ResolutionError#PAYMENT_MODE_NOT_SUPPORTED} — rows match the
 *       direction, but none is wired for the requested presentment mode.</li>
 * </ol>
 *
...
public class LocationSchemeResolver {
     ...
     *
     * @throws SchemeResolutionException carrying the most specific
     *         {@link ResolutionError} branch that applies.
     */
    public SchemeResolution resolve(LocationSchemeQuery query) {
        ...
        if (rows.isEmpty()) {
            throw new SchemeResolutionException(ResolutionError.NO_SCHEME_FOR_LOCATION,
        ...
        if (directionMatches.isEmpty()) {
            throw new SchemeResolutionException(ResolutionError.DIRECTION_NOT_ENABLED,
        ...
        if (candidates.isEmpty()) {
            throw new SchemeResolutionException(ResolutionError.PAYMENT_MODE_NOT_SUPPORTED,
    ...
    private static String validate(LocationSchemeQuery query) {
        if (query == null) {
            throw new SchemeResolutionException(ResolutionError.VALIDATION_ERROR, "query required");
        }
