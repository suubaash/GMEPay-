 ...
 * Flyway applies V001-V003 from the H2-PG-mode datasource in application.properties.
 */
@DataJpaTest
...
class CpmSessionPersistenceH2SliceTest {
    ...
    void saveThenFindByPaymentId() {
        ...
        store.save(token, "inbound", "KR", "cust", false);
    ...
    void expireOverdueMarksOnlyPastIssuedTokens() {
        ...
        List<String> expired = store.expireOverdue(now);

        assertEquals(List.of("TOK-EXP"), expired);
    ...
    void expireOverdueIsIdempotent() {
        ...
        assertEquals(List.of("TOK-I"), store.expireOverdue(now));
