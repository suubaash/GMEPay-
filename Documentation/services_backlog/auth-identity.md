...
These tickets close Partner Setup audit gaps under the 8-slice vertical plan in `docs/PARTNER_SETUP_PLAN.md` (approved 2026-06-11). Each ticket id `21.{slice}-Pxx` maps to a wizard slice; ADR references point at `docs/adr/`. Tickets owned by **auth-identity** live here; cross-service contributions are listed at the bottom for awareness.
...
### 21.1-P14 — Keycloak OAuth2 resource-server config on api-gateway and config-registry (retire password=demo)
*Slice:* **1** · *Est:* 90 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* ADR-011

**Context.** Per ADR-011 Keycloak runs as the IDP (docker-compose service, Rocky 9 base image, Postgres-backed). api-gateway becomes an OAuth2 resource server validating Keycloak-issued JWTs. ops-partner-bff's `password=demo` AuthController is retired (becomes a 410 Gone with a Keycloak redirect URL in the response body).

**Steps.** Add docker-compose service `keycloak` (image quay.io/keycloak/keycloak on Rocky 9 base) with Postgres datasource; bootstrap realm `gmepay` with client `admin-ui-spa` (public, PKCE) and `bff-confidential`; api-gateway SecurityConfig switches to `.oauth2ResourceServer(o -> o.jwt(j -> j.jwkSetUri(env.KEYCLOAK_JWKS_URL)))`; config-registry switches the same way; remove `services/ops-partner-bff/src/main/java/com/gme/pay/bff/auth/DemoAuthController.java` body and return 410 Gone with `Location: ${KEYCLOAK_AUTH_URL}`.

**Deliverable.** `docker/compose/docker-compose.yml; services/api-gateway/src/main/java/com/gme/pay/gateway/SecurityConfig.java; services/config-registry/src/main/java/com/gme/pay/config/security/SecurityConfig.java; services/ops-partner-bff/.../DemoAuthController.java`

**Acceptance.**
- docker compose up brings keycloak up healthy on :8080 with realm gmepay seeded
- JWT issued by Keycloak with role=OPS_ADMIN authenticates against api-gateway
- Old `POST /v1/auth/login` returns 410 Gone with Location header pointing to Keycloak
- Tampered/expired JWTs return 401 from api-gateway and config-registry

### Slice 8 tickets owned by this service

### 21.8-P01 — Wire ApiKeyEntity + PrincipalEntity into partner-create transaction (auth-identity)
*Slice:* **8** · *Est:* 90 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* —

**Context.** Slice 8 — credential wire-up debt. On partner activation auth-identity must generate API key + HMAC secret and store the bcrypt hash. Plaintext returned ONCE in the activation response.

**Steps.** Extend services/auth-identity/src/main/java/com/gme/pay/auth/CredentialIssuer.java with issueOnActivation(Long partnerId, Environment env); generates UUIDv4 api_key (with prefix pk_test_ for SANDBOX, pk_live_ for PRODUCTION); SecureRandom 32 bytes for HMAC secret; BCryptPasswordEncoder.encode for storage; insert into api_key + principal tables; returns SecretIssuance(apiKey, plaintextHmacSecret) ONCE.

**Deliverable.** `services/auth-identity/src/main/java/com/gme/pay/auth/CredentialIssuer.java`

**Acceptance.**
- Activation response contains apiKey starting with pk_test_ for SANDBOX
- api_secret_hash in DB starts with $2a$ (bcrypt)
- Subsequent GET /v1/credentials/{partnerId} never returns plaintext
- Issuance audit-logged with actor=system, action=CREATE

### 21.8-P02 — partner_ip_allowlist table + api-gateway pre-signature 403 enforcement
*Slice:* **8** · *Est:* 75 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* —

**Context.** IP allowlist with up to 10 CIDRs per partner. api-gateway checks before HMAC validation (pre-signature 403 is faster + less log-noise).

**Steps.** V231__partner_ip_allowlist.sql: id, partner_id FK, cidr CIDR NOT NULL, label VARCHAR(80); CHECK count(*) per partner <= 10 (app-layer); update services/api-gateway/src/main/java/com/gme/pay/gateway/security/IpAllowlistFilter.java to load allowlist from config-registry, cache in Caffeine for 60s; return 403 with code IP_NOT_ALLOWLISTED before any signature check.

**Deliverable.** `services/config-registry/src/main/resources/db/migration/V231__partner_ip_allowlist.sql; services/api-gateway/src/main/java/com/gme/pay/gateway/security/IpAllowlistFilter.java`

**Acceptance.**
- Partner with 10 CIDRs: 11th raises 422 MAX_ALLOWLIST_REACHED
- Request from non-allow-listed IP returns 403 before HMAC validation runs
- Request from allow-listed IP proceeds to HMAC validation
- Cache TTL ≤ 60s — change reflected within a minute

### 21.8-P03 — mTLS cert exchange: partner_mtls_cert table + Nginx mTLS verification
*Slice:* **8** · *Est:* 90 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* ADR-002

**Context.** Per ADR-002 the edge is Nginx. mTLS verification happens at the Nginx layer with the partner's cert pinned via partner_mtls_cert.fingerprint_sha256.

**Steps.** V232__partner_mtls_cert.sql: id, partner_id FK, cert_pem TEXT, fingerprint_sha256 BYTEA UNIQUE, expiry_date DATE, status CHECK IN (ACTIVE,REVOKED,EXPIRED); bitemporal; new Nginx config snippet `ops/nginx/mtls.conf` mapping fingerprint -> partner via Lua + Redis; api-gateway header X-Client-Cert-Fingerprint must match.

**Deliverable.** `services/config-registry/src/main/resources/db/migration/V232__partner_mtls_cert.sql; ops/nginx/conf.d/mtls.conf`

**Acceptance.**
- Connection with valid cert: header X-Client-Cert-Fingerprint set
- Expired cert (expiry_date < now): Nginx returns 403
- Unknown fingerprint: Nginx returns 403
- Revoked cert: Nginx returns 403 within 60s of status flip

### 21.8-P04 — Sandbox vs production credential pairs (partner_credential_environment enum)
*Slice:* **8** · *Est:* 60 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* —

**Context.** Each partner gets two credential pairs: SANDBOX (pk_test_, sk_test_) + PRODUCTION (pk_live_, sk_live_). Cross-environment use fails.

**Steps.** Add environment VARCHAR(15) CHECK IN (SANDBOX,PRODUCTION) to api_key table (V233); CredentialIssuer emits both pairs on activation; api-gateway resolves the environment from the api_key prefix and rejects if env != target endpoint env (sandbox.gmepay.com vs api.gmepay.com).

**Deliverable.** `services/auth-identity/src/main/resources/db/migration/V233__api_key_environment.sql; services/api-gateway/src/main/java/com/gme/pay/gateway/security/EnvironmentResolver.java`

**Acceptance.**
- pk_test_ presented to api.gmepay.com (production endpoint) returns 403 ENVIRONMENT_MISMATCH
- pk_live_ presented to sandbox.gmepay.com returns 403
- Activation response includes both apiKeys and both plaintext secrets (one-time)
- Cross-env use audit-logged

### 21.8-P05 — Credential rotation scheduled job: propose CR at 11 months + revoke endpoint
*Slice:* **8** · *Est:* 75 min · *Role:* Backend · *Owner:* auth-identity · *ADR refs:* —

**Context.** 12-month rotation per SEC-09. At 11 months a scheduled job creates a PROPOSED change_request asking the operator to rotate; revoke endpoint takes a credential out of use with audit_log entry.

**Steps.** Add `CredentialRotationScheduler.java @Scheduled(cron="0 0 2 * * *")` to auth-identity; SELECT api_key WHERE issued_at < now() - 11 months AND status=ACTIVE; create change_request via config-registry; add POST /v1/admin/credentials/{id}/revoke endpoint (admin-only, audit-logged); on revoke set status=REVOKED and emit `gmepay.credential.revoked` event.

**Deliverable.** `services/auth-identity/src/main/java/com/gme/pay/auth/CredentialRotationScheduler.java; services/auth-identity/src/main/java/com/gme/pay/auth/CredentialRevokeController.java`

**Acceptance.**
- Cred issued > 11 months: nightly job creates PROPOSED CR exactly once
- Operator approves CR + new credential issued; old credential set to status=ROTATED
- Revoke endpoint immediately invalidates the key (api-gateway sees revoked within 60s)
- Audit log row written for every rotation and revoke
