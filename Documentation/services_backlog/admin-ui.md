...
These tickets close Partner Setup audit gaps under the 8-slice vertical plan in `docs/PARTNER_SETUP_PLAN.md` (approved 2026-06-11). Each ticket id `21.{slice}-Pxx` maps to a wizard slice; ADR references point at `docs/adr/`. Tickets owned by **admin-ui** live here; cross-service contributions are listed at the bottom for awareness.
...
### 21.1-P15 — Wizard scaffold (8 steps as stepper, server-side draft persistence)
*Slice:* **1** · *Est:* 120 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-012

**Context.** Slice 1 — admin-ui wizard scaffold. 8 tabs/steps; navigation gated by server-side draft state.

**Steps.** Add `apps/admin-ui/src/app/partners/new/wizard/` with steps directory; use Next.js App Router parallel routes for the stepper; persist current step via PATCH to /v1/admin/partners/draft/{id}; Yup validation per step; navigation guard: cannot advance to step N+1 with invalid step N.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/page.tsx; apps/admin-ui/src/app/partners/new/wizard/components/Stepper.tsx`

**Acceptance.**
- Stepper renders 8 numbered tabs with click-to-jump for completed steps
- Refreshing the browser mid-wizard resumes at the same step
- Vitest covers stepper navigation gating
- Accessibility: tab order keyboard-navigable

### 21.1-P16 — Step 1 (Identity) form with regional tax-ID validation
*Slice:* **1** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** First wizard step. Mirrors backend P08+P09 validators.

**Steps.** Form fields per ADR-010 partner aggregate; Yup schema with KR_BRN checksum, SG_UEN format, etc.; submits PATCH /v1/admin/partners/draft/{id}/step-1; on 422 displays field-level error from unmet[].

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-1-identity/page.tsx`

**Acceptance.**
- KR_BRN bad checksum highlighted inline before submit
- Submit with valid body advances to step 2
- Server-side 422 errors rendered next to the offending field
- All field labels in Korean + English (UI locale toggle)

### 21.1-P17 — Draft list on /partners (existing partners + drafts side-by-side with Resume button)
*Slice:* **1** · *Est:* 60 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Partners list now shows two sections: Active partners + In-progress drafts.

**Steps.** Edit apps/admin-ui/src/app/partners/page.tsx to fetch GET /v1/admin/partners + GET /v1/admin/partners/drafts in parallel; render two tables; Resume button links to /partners/new/wizard?draftId=X.

**Deliverable.** `apps/admin-ui/src/app/partners/page.tsx`

**Acceptance.**
- Both tables render with paginated results
- Resume button jumps into the wizard at the correct step
- Empty drafts section renders a 'Start new partner' CTA
- Loading skeletons display before data lands

### 21.1-P18 — Keycloak OIDC login replacing password=demo (admin-ui auth)
*Slice:* **1** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-011

**Context.** Per ADR-011 the Keycloak SPA client (`admin-ui-spa`, PKCE) replaces the demo password form.

**Steps.** Install @react-keycloak/web or use authjs.dev with Keycloak provider; wrap app with KeycloakProvider; on login redirect to Keycloak; on logout call Keycloak end-session-endpoint; refresh token automatic; existing 'password=demo' form deleted.

**Deliverable.** `apps/admin-ui/src/app/providers.tsx; apps/admin-ui/src/lib/auth/keycloak.ts`

**Acceptance.**
- Loading the app redirects to Keycloak login
- After login JWT carried in Authorization header to BFF
- Token refresh transparent (no 401 visible to user)
- Logout clears the Keycloak session + admin-ui session

### Slice 2 tickets owned by this service

### 21.2-P06 — Step 2 (Contacts) multi-row editor with E.164 phone validation
*Slice:* **2** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Multi-row contact editor; libphonenumber-js for E.164 validation.

**Steps.** Form with add-row/remove-row controls; per-row role dropdown, name, email, phone, signatory checkbox, notes; client-side validation libphonenumber-js parsePhoneNumber(); submits PATCH /v1/admin/partners/draft/{id}/step-2.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-2-contacts/page.tsx`

**Acceptance.**
- Phone '01012345678' (KR) auto-formats to '+821012345678'
- Adding a row appends to local state; remove-row works
- ≥1 row required to advance
- Duplicate (role,email) flagged inline

### 21.2-P07 — Approval queue page + Approve/Reject UI with reason field
*Slice:* **2** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-008

**Context.** Checker UI for the 4-eyes workflow. Lists PROPOSED change_requests; click Approve/Reject.

**Steps.** New page apps/admin-ui/src/app/approvals/page.tsx; fetches GET /v1/admin/partners/change-requests?state=PROPOSED; filters by current operator's role; Approve modal includes optional reason; Reject modal requires reason; on success the row disappears.

**Deliverable.** `apps/admin-ui/src/app/approvals/page.tsx`

**Acceptance.**
- Approver sees only requests they did not propose (self-approval pre-filtered)
- Approve click triggers POST /change-requests/{id}/approve
- Reject without reason is disabled (button greyed)
- Audit-log link for each row jumps to the audit viewer

### 21.2-P08 — Audit log viewer on partner detail page (read-only, paginated)
*Slice:* **2** · *Est:* 60 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-007

**Context.** Per partner audit log timeline. Each row shows actor, action, before/after JSON diff.

**Steps.** New component apps/admin-ui/src/app/partners/[id]/audit/page.tsx; fetches GET /v1/admin/partners/{id}/audit-log; renders a diff view (react-json-view) for before/after; pagination via RFC 5988 link headers.

**Deliverable.** `apps/admin-ui/src/app/partners/[id]/audit/page.tsx`

**Acceptance.**
- Timeline orders by occurred_at desc
- JSON diff highlights changed fields
- Pagination Next/Prev buttons work
- Empty state renders 'No audit events yet'

### Slice 3 tickets owned by this service

### 21.3-P11 — Step 3 (KYB) form: license, UBO entry table, document upload, Run screening
*Slice:* **3** · *Est:* 120 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-006, ADR-009

**Context.** Step 3 — KYB. Largest UI step. UBO table with name + ownership % + PEP toggle + country. Drag-drop document upload. Run screening button.

**Steps.** Form with: license fields, UBO multi-row, document upload (react-dropzone), Wolfsberg CBDDQ slot, Run-screening button calling PATCH step-3 with runScreening=true; screening result panel below.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-3-kyb/page.tsx`

**Acceptance.**
- UBO sum > 100% highlighted with error
- Document upload shows progress + thumbnail
- Run screening shows in-progress spinner; on complete shows decision + hits
- Re-screen button visible after first screening

### 21.3-P12 — Document viewer modal (with version history)
*Slice:* **3** · *Est:* 60 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Click a document row -> modal with PDF preview + version history (each row shows version, uploaded_by, uploaded_at, expiry_date).

**Steps.** Component apps/admin-ui/src/app/partners/[id]/documents/DocumentViewerModal.tsx using react-pdf; version history table; Download link presigned URL via BFF.

**Deliverable.** `apps/admin-ui/src/app/partners/[id]/documents/DocumentViewerModal.tsx`

**Acceptance.**
- PDF renders inline
- Version history shows 2 rows after second upload
- Download button fetches presigned URL + opens in new tab
- Expired document shows red badge

### Slice 4 tickets owned by this service

### 21.4-P07 — Step 4 (Banking & Settlement) bank-account multi-row editor + settlement preview
*Slice:* **4** · *Est:* 120 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Bank account multi-row editor with per-row verification button + settlement-config panel showing live preview ('Mon 11:30 KST -> Wed 11:30 KST').

**Steps.** Form per partner_bank_account fields; per-row Verify button POSTs to verification endpoint; settlement-config card with cycle/cutoff/method/timezone picker; preview pane calls GET /v1/admin/partners/draft/{id}/settlement-preview.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-4-banking/page.tsx`

**Acceptance.**
- Two bank accounts in different ccys render side by side
- Verify button toggles UNVERIFIED -> KFTC_VERIFIED on success
- Settlement preview updates live when cycle changes
- Settlement method BAKONG + currency KRW shows inline warning

### Slice 5 tickets owned by this service

### 21.5-P06 — Step 5 (Prefunding) form (OVERSEAS only) + prefunding dashboard tile on partner detail
*Slice:* **5** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Step 5 visible only when partner_type=OVERSEAS. Sliders + tier preview. Dashboard tile post-activation: balance gauge + tier marker.

**Steps.** Add apps/admin-ui/src/app/partners/new/wizard/step-5-prefunding/page.tsx hidden behind partner_type check; dashboard tile in [id]/page.tsx using recharts gauge; threshold markers at 70/85/95%.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-5-prefunding/page.tsx; apps/admin-ui/src/app/partners/[id]/PrefundingTile.tsx`

**Acceptance.**
- Step 5 hidden for LOCAL partner
- Tier sliders enforce 70 < 85 < 95
- Float top-up bank picker filtered to FLOAT_TOPUP accounts
- Gauge fills proportionally to current balance

### Slice 6 tickets owned by this service

### 21.6-P11 — Step 6 (Commercial Terms) form: ccy split + rule editor + fees + FX + limits + contract
*Slice:* **6** · *Est:* 180 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Largest UI step. Multi-section form with 6 panels.

**Steps.** Form panels: ccy split picker, rule editor (per scheme x direction with margin sliders), fee schedule (fixed+bps+optional tier table), FX margin slider + reference source picker + quote-hold input, limits with 소액해외송금업 cap markers visible, contract dates + auto-renewal.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-6-commercial/page.tsx`

**Acceptance.**
- Rule editor enforces m_a+m_b >= 2% inline (before submit)
- Fee tier ranges visualised on a slider; overlapping flagged
- FX margin slider shows preview rate in real-time
- 소액해외송금업 caps displayed as red markers on the limit sliders

### Slice 7 tickets owned by this service

### 21.7-P06 — Step 7 (Schemes & Corridors) matrix editor + ZeroPay drill-down + corridor builder
*Slice:* **7** · *Est:* 120 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Scheme matrix (rows=schemes, cols=direction x role) + ZeroPay drill-down (merchantId, sub-merchantId, institution code, D/I, approval method per CPM/MPM) + corridor matrix builder.

**Steps.** Page with two main sections: scheme matrix (checkbox grid) and corridor matrix; per-row drill-down opens a side panel with the scheme-specific fields; operating-hours preview rendered as a 24h clock visualization.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-7-schemes/page.tsx`

**Acceptance.**
- Toggling a scheme on opens the side panel
- ZeroPay drill-down requires zeropay_merchant_id (yellow border until filled)
- Corridor builder validates src_country != dst_country
- Operating-hours preview reflects KFTC 16:30 KST cutoff for ZeroPay

### Slice 8 tickets owned by this service

### 21.8-P18 — Step 8 (Review + Activate) + one-time credential modal (apiKey + HMAC + webhook secret)
*Slice:* **8** · *Est:* 120 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Final step. Review tab with anchor jumps to earlier steps. Activate button calls activation endpoint. On success a credential modal displays the three secrets with copy-to-clipboard and a Confirm-I-stored-these dismissal.

**Steps.** Add apps/admin-ui/src/app/partners/new/wizard/step-8-review/page.tsx; review panel uses partner-draft state; Activate button calls POST /v1/admin/partners/draft/{id}/activate; on 200 with credential block show one-time modal with copy buttons + checkbox 'I have stored these securely' required to dismiss.

**Deliverable.** `apps/admin-ui/src/app/partners/new/wizard/step-8-review/page.tsx; apps/admin-ui/src/app/partners/new/wizard/CredentialModal.tsx`

**Acceptance.**
- Activate with unmet gates surfaces a list of unmet items linked to the relevant step
- Credential modal blocks dismissal until checkbox ticked
- Copy-to-clipboard for each secret
- After dismissal those secrets are unrecoverable via UI (confirmed by re-opening modal -> only hashes shown)

### 21.8-P19 — Partner detail page: status header with FSM transitions menu + Suspend/Reactivate/Terminate actions
*Slice:* **8** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* ADR-008

**Context.** Post-activation detail page. Status badge + dropdown of allowed FSM transitions. Suspend requires reason. Reactivate requires checker approval.

**Steps.** Edit apps/admin-ui/src/app/partners/[id]/page.tsx; new StatusHeader component showing current status + dropdown of allowed transitions per FSM; Suspend modal requires reason dropdown (LIMIT_BREACH, SANCTIONS_HIT, ...); Reactivate triggers approval queue entry.

**Deliverable.** `apps/admin-ui/src/app/partners/[id]/page.tsx; apps/admin-ui/src/app/partners/[id]/StatusHeader.tsx`

**Acceptance.**
- Status badge color matches state (green=LIVE, amber=SUSPENDED, red=TERMINATED)
- Transitions menu hides disallowed transitions per FSM
- Suspend modal requires reason; submits PROPOSED change_request
- Reactivate proposes; appears in approval queue for checker

### 21.8-P20 — Credential rotation panel + BOK/Hometax/KoFIU reporting settings tab
*Slice:* **8** · *Est:* 90 min · *Role:* Frontend · *Owner:* admin-ui · *ADR refs:* —

**Context.** Post-activation panels for credential rotation history + the regulatory reporting settings tab.

**Steps.** Add tabs to partners/[id]/page.tsx: Credentials (rotation history + Rotate Now button), Reporting (BOK/Hometax/KoFIU settings forms); Rotate Now creates a change_request; settings forms PATCH the relevant partner fields.

**Deliverable.** `apps/admin-ui/src/app/partners/[id]/credentials/page.tsx; apps/admin-ui/src/app/partners/[id]/reporting/page.tsx`

**Acceptance.**
- Rotate Now opens confirmation dialog -> creates CR
- Old credentials show status=ROTATED with rotated_at
- BOK fx_reporting_category dropdown shows only allowed enum values
- Hometax issuer cert upload uses the document upload flow
